...
values
(11,'Marketing', 'Pune', 'Rakesh Mehta', '1300000');
...
insert into employees(firstname, lastname, departmentid, salary, dateofjoining, email)
values
('Riya', 'Sharma', 1, 50000, '2024-01-01', 'riya.sharma@example.com'),
...
('Nitin', 'Shah', 7, 68000, '2024-03-03', 'kunal.singh@example.com');
...
select departmentid, min(DateOfJoining) as earliestDateOfJoining from employees
group by departmentid ;
...
SELECT 
LEFT(firstname, 1) AS Starting_Letter,
COUNT(*) AS Employee_Count
FROM employees
GROUP BY LEFT(firstname, 1)
ORDER BY Starting_Letter ASC;
...
select location as city, count(*) from departments
group by location;
...
select location as city, sum(annualbudget) as totalbudget, count(*) from departments
group by location;
...
select location as city, avg(annualbudget) from departments
group by location;
...
select location as city, max(annualbudget) as maxannualbudget,  min(annualbudget) as minannualbudget from departments
group by location;
...
firstname,
CONCAT(phone, 
       IF(extension IS NOT NULL, CONCAT(' Ext: ', extension), '')
) AS Contact_Number
FROM employees;
  
-- Situation -6: To display the complete address, concatenate the street, city, and  postalcode of each employee;
...
-- Situation -8: For sending out a personalized notification, concatenate the employee's department with their position;
  ...
  SELECT 
CONCAT(d.DepartmentName, ' - ', e.Position) AS PersonalizedNotification
FROM Employees e
JOIN Departments d
...
select concat(year(dateofjoining), ' - ', firstname) as annualreport
from employees;
...
select employeeid, firstname, lastname, contactnumber from employees
where  EmployeeID = 120;
...
select employeeid, firstname, lastname, projectassignment from employees
where  employeeid = 110;
...
select employeeid, firstname, lastname, dateofjoining from employees
where  employeeid = 103;
...
-- Situation -5: After the closure of , we need to delete all records of employees who 
...
select * from employees
where email is null and contactnumbers is null;
...
select employeeid, firstname, lastname, maritalstatus from  employees;
...
select employeeid, firstname, lastname, EmployeeCategory from  employees
limit 10;
...
alter table staff
change columnTeamID DepartmentID int; 
