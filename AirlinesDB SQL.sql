...
SELECT * FROM Flights
where departurecity = 'Dubai International Airport' ;
...
--1. Update the ticket price of the flight with FlightID = 12 by increasing it by 10%;

select * from flights
where flightid = 12;

update  flights
set ticketprice = ticketprice * 1.1
where flightid = 12 ;

--2. Change the arrival city of all flights arriving in ‘Delhi’ to ‘New Delhi’, but only if TicketPrice > 5000.


--3. Update the airline name to ‘Air India Express’ for flights whose airline was previously ‘Air India’.


--4. Reduce available seats by 5 for all flights departing from ‘Mumbai’.


--5. Set TicketPrice = TicketPrice + 2000 for flights operated by 'IndiGo'.


DELETE Queries

--6. Delete all flights that have TicketPrice less than 3000;
...
--7. Delete the flight with FlightID = 20;
...
--8. Delete flights where AvailableSeats = 0;
...
--9. Delete flights arriving in 'Chennai' but only if TicketPrice > 8000;

select * from flights
where arrivalcity = chennai ;

delete from flights
where TicketPrice > 8000;

--10. Delete all flights operated by ‘SpiceJet’;

select * from flights
where operatorname = ‘SpiceJet’;

delete from flights
where operatorname = ‘SpiceJet’;

SELECT Queries (Important)

--11. Retrieve flights where TicketPrice is between 5000 and 10000.


--12. List all flights with available seats greater than 50.


--13. Show airlines and the number of flights operated by each.


--14. Retrieve the maximum ticket price for each arrival city.


--15. Display flights ordered by TicketPrice in descending order.


--16. Show all flights where departure city and arrival city are the same.


--17. Retrieve the average available seats for flights operated by 'Vistara'.


--18. List flights where TravelTime > 3 hours.


--19. Show total revenue (AvailableSeats × TicketPrice) for each airline.


--20. Find departure cities with more than 5 flights.
